    ...
    return __generator(this, function (_b) {
      switch (_b.label) {
        ...
        case 1:
          ...
          audioSrc.connect(analyser).connect(audioCtx.destination); // osc = audioCtx.createOscillator();
...
function waveMap(buffer) {
  ctx2.clearRect(0, 0, $canvas2.width, $canvas2.height);

  if (buffer) {
    var p = audioCtx.currentTime / buffer.duration;
    var steps = $canvas2.width;
    var maxH = $canvas2.height / buffer.numberOfChannels;

    for (var i = 0; i < buffer.numberOfChannels; i++) {
      var bytes = buffer.getChannelData(i);
      var pv = p * $canvas2.width;
      var width = $canvas2.width / steps;

      for (var d = 0; d < bytes.length; d += steps) {
        var x = width * d / steps;
        var h = bytes[d] * maxH;
        var y = maxH / 2 - h / 2 + maxH * i;
        var color = x <= pv ? 'white' : 'rgba(255, 255, 255, 0.25)';
...
function loop() {
  ...
  if (frequencyBytes) {
    ...
    var bytes = frequencyBytes.slice(0, -45);
    waveMap(audioSrc.buffer);

    for (var i = 0; i < bytes.length; i++) {
      var width = $canvas.width / bytes.length;
      var x = width * i;
      var intensity = bytes[i] / 256;
      var h = intensity * ($canvas.height - 32);
      var color = "rgb(" + (1 - i / bytes.length) * 255 + ", " + i / bytes.length * 255 + ", 51)";
      ctx.fillStyle = color;
      ctx.fillRect(x, $canvas.height / 2 - h / 2, width, h); // ctx.fillRect($canvas.width - 1, $canvas.height - x, width, 1);
